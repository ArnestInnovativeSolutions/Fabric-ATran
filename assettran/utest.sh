...
docker exec -e "CORE_PEER_LOCALMSPID=atranMSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/atran.arnest.in/users/Admin@atran.arnest.in/msp" cli peer chaincode install -n atrancc -v 1.1 -p "$CC_SRC_PATH" -l "$CC_RUNTIME_LANGUAGE"
#docker exec -e "CORE_PEER_LOCALMSPID=atranMSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/atran.arnest.in/users/Admin@atran.arnest.in/msp" cli peer chaincode instantiate -o orderer.arnest.in:7050 -C atranchannel -n atrancc -l "$CC_RUNTIME_LANGUAGE" -v 1.1 -c '{"Args":[]}' -P "OR ('atranMSP.member', 'arMSP.member' )"
#sleep 10
